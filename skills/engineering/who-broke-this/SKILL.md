---
name: who-broke-this
description: Bounded Codex review loop — a pinned Codex reviewer checks staged changes, then performs focused verification of unstaged fixes with user-gated extensions. Only run when explicitly invoked via /who-broke-this or $who-broke-this.
...
- If `codex` is unavailable, tell the user how to install it (`npm install -g @openai/codex`) and
  offer a degraded fallback: one review round performed by you directly against the staged diff,
  same triage standard, no loop. Only proceed with the fallback if the user accepts.
...
RUN="$(mktemp -d "${TMPDIR:-/tmp}/who-broke-this-${BRANCH:-detached}-$(date +%Y%m%d-%H%M%S).XXXXXX")"
...
**House-stack check (round 1 only):** if the repo is an Nx monorepo with Prisma (`nx.json` at
root plus a `prisma/` dir or `prisma.config.ts`) or an Expo app (`app.config.ts` + `expo` in
package.json), append the contents of `$SKILL_DIR/references/stack-bug-classes.md` under the
`## Additional focus` heading as well — it lists house-stack breakage patterns a generic review
misses. Otherwise omit it.
...
Reviews at `xhigh` effort run **~10–15 minutes per round**. Set the command timeout to `600000`
milliseconds and tell the user when a round starts. Do not silently start an unapproved extension.
